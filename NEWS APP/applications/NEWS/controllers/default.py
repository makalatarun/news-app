...
def delete_category():
	id1=int(request.args(0))
	x=db(db.news.Category==id1).select(db.news.ALL)
	for k in x:
		id2=k['id']
		db(db.LD.new_id==id2).delete()
		db(db.comments.new_id==id2).delete()
		db(db.news.Category==id1).delete()
	db(db.category.id == id1).delete()
...
@auth.requires_login()
def list_cat():
	m = db(db.category.id > 0).select(db.category.ALL)
	y=[]
	for k in m:
		x={}
		x['name']=k['name']
